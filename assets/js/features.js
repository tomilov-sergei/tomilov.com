// Feature runtime: loaded by the small route-aware bootstrap only when a page needs it.
...
function initHomeCanvasVisibility(viewport, surface) {
  const nodes = [...surface.querySelectorAll(".home-canvas-node")].map((node) => ({
    node,
    x: Number.parseFloat(node.style.left) || 0,
    y: Number.parseFloat(node.style.top) || 0,
    width: node.offsetWidth || 224,
...
async function hydrateHomeCanvasNode(node, mediaLimit = 0) {
  if (node.classList.contains("is-hydrated") || node.dataset.canvasHydrating) {
    return { loadedMedia: 0 };
  }

  node.dataset.canvasHydrating = "true";
  let template = node.querySelector("[data-canvas-card-template]");

  if (!template) {
    const html = await loadHomeCanvasNodeContent(node);
    if (!html) {
      delete node.dataset.canvasHydrating;
      node.classList.add("has-missing-content");
      return { loadedMedia: 0 };
    }
  ...
  node.querySelector(".visually-hidden")?.remove();
  if (template.parentNode === node) {
...
async function loadHomeCanvasNodeContent(node) {
  const lang = currentLanguage();
  const [chunk, contentKey] = String(node.dataset.canvasCard || "").split("|", 2);
  if (!chunk || !contentKey) return "";

  const cacheKey = `${lang}:${chunk}`;
  let request = homeCanvasContentChunks.get(cacheKey);

  if (!request) {
    request = fetch(`/assets/canvas/${lang}-${chunk}.json`, {
      headers: { Accept: "application/json" },
    }).then((response) => {
      if (!response.ok) throw new Error(`Canvas chunk ${chunk}: HTTP ${response.status}`);
      return response.json();
    });
    homeCanvasContentChunks.set(cacheKey, request);
  }
  ...
  try {
    ...
    homeCanvasContentChunks.delete(cacheKey);
    console.error(error);
    return "";
  }
}
...
function createHomeCanvasPostOverlay(root, lang) {
  ...
  overlay.setAttribute("aria-label", lang === "en" ? "Canvas post" : "Пост на холсте");
...
async function swapPage(url, options = {}) {
  ...
  if (document.startViewTransition && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    document.startViewTransition(updateDocument);
  } else {
    updateDocument();
  }
}
...
async function initTelegramFeed(feedElement) {
  ...
  const topicButtons = [...document.querySelectorAll("[data-post-topic]")];
  const result = document.querySelector("[data-post-result]");
  const batchSize = 18;
  let posts = [];
  let visiblePosts = [];
  let rendered = 0;
  ...
  try {
    ...
    for (const button of topicButtons) {
      button.addEventListener("click", () => {
        ...
        for (const sibling of topicButtons) {
  ...
  function applyFilters() {
    const query = normalizeText(searchInput?.value).toLocaleLowerCase(lang === "en" ? "en" : "ru");
    visiblePosts = posts.filter((post) => {
      if (activeTopic !== "all" && getPostTopic(post) !== activeTopic) return false;
      if (!query) return true;
      const haystack = [
        getPostText(post, lang),
        ...(post.entities || []).map((entity) => `${entity.text || ""} ${entity.href || ""}`),
      ].join(" ").toLocaleLowerCase(lang === "en" ? "en" : "ru");
      return haystack.includes(query);
    });

    feedElement.replaceChildren();
    feedElement.removeAttribute("data-static-post-feed");
    rendered = 0;
    renderNextBatch();

    if (result) {
      result.textContent = lang === "en"
        ? `${visiblePosts.length} observations`
        : `${visiblePosts.length} наблюдений`;
    }
  }

  function renderNextBatch() {
    const fragment = document.createDocumentFragment();
    const nextPosts = visiblePosts.slice(rendered, rendered + batchSize);
    ...
    if (loadMore) {
      loadMore.hidden = rendered >= visiblePosts.length;
    }
  }
}

function getPostTopic(post) {
  const text = normalizeText(`${post.text || ""} ${post.translations?.en?.text || ""}`).toLocaleLowerCase("ru");
  const topics = [
    ["ai", [/(^|\s)(ai|ии)(\s|$)/i, /gpt|openai|нейро|llm|chatgpt|midjourney|claude|генератив|agent|агент|codex/i]],
    ["photos", [/фото|камера|снимок|снимки|leica|iphone|hdr|объектив|пл[её]нк|фотограф|съ[её]мк/i]],
    ["products", [/продукт|стартап|сервис|прилож|пользователь|фича|запуск|подписк|монетиз|платформ|рекомендац/i]],
    ["design", [/дизайн|интерфейс|\bui\b|\bux\b|figma|шрифт|визуал|лендинг|экран|кнопк|цвет|типограф|анимац|микро/i]],
    ["games", [/игр|\bgame\b|gaming|doom|silent hill|nintendo|playstation|xbox|steam|sekiro|dead space|гейм|mixtape|wicked/i]],
    ["brands", [/бренд|\bbrand\b|nike|apple|google|teenage engineering|dyson|sony|tesla|ikea|leica|nothing|airbnb|ferrari|anthropic/i]],
  ];
  let bestTopic = "products";
  let bestScore = 0;

  for (const [topic, patterns] of topics) {
    const score = patterns.reduce((total, pattern) => total + Number(pattern.test(text)), 0);
    if (score > bestScore) {
      bestTopic = topic;
      bestScore = score;
    }
...
function createPost(post, lang) {
  ...
  const permalink = document.createElement("a");
  permalink.className = "screenshot-permalink";
  permalink.href = `${lang === "en" ? "/en" : ""}/screenshots/${post.id}/`;
  permalink.textContent = lang === "en" ? "Open note" : "Открыть заметку";
...
function createPhotoCard(photo, index, lang) {
  ...
  const img = document.createElement("img");
  const previewWidths = [480, 960, 1440].filter((width) => !photo.width || width <= Number(photo.width));
  let imageContainer = img;
  if (photo.hdr || !previewWidths.length) {
    img.src = photo.src;
  } else {
    const defaultWidth = [...previewWidths].reverse().find((width) => width <= 960) || previewWidths[0];
    img.src = getPhotoFeedPreviewUrl(photo, defaultWidth);
    img.srcset = previewWidths
      .map((width) => `${getPhotoFeedPreviewUrl(photo, width)} ${width}w`)
      .join(", ");
    img.sizes = "(max-width: 719px) calc(100vw - 24px), (max-width: 1199px) 45vw, 360px";
    img.dataset.fallbackSrc = photo.src;

    const source = document.createElement("source");
    source.type = "image/webp";
    source.srcset = previewWidths
      .map((width) => `${getPhotoFeedPreviewUrl(photo, width, "webp")} ${width}w`)
      .join(", ");
...
function getPhotoFeedPreviewUrl(photo, edge, extension = "jpg") {
  const id = String(photo.id || "photo").replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^[.-]+|[.-]+$/g, "") || "photo";
  return `/assets/photos/feed/${id}-${edge}.${extension}`;
}
