...
export function createStudioLighting(renderer){
  ...
  let selected=lightPresets[0];
  function texture(preset){
    if(cache.has(preset.id))return cache.get(preset.id).texture;
    const room=new THREE.Scene();room.background=new THREE.Color(preset.ambient);
    ...
    cache.set(preset.id,target);return target.texture;
  }
  return {
    ...
    apply(scene,materials,enabled){
      ...
      for(const {material} of saved){
        ...
        material.envMap=map;material.envMapRotation.set(0,0,0);material.envMapIntensity=1;
      }
      return ()=>{
