...
async function build(){
  ...
  const fallback = new Map(decoded.map(asset => [asset.original, new URL(`reference/decoded/${asset.file}`,location.href).href]));
...
function applyExplode(){
  const k = explodeAmount * EXPLODE_K;
  for (const p of parts) p.mesh.position.copy(p.base).addScaledVector(p.dir, k);
}
...
const poseNames={PT_Landscape:'Landscape',PT_Portrait:'Portrait',PT_Closed:'Closed',PT_Laptop:'Seated',PT_Tent:'Standing',PT_Durability:'Durability'};
...
for(const [label,name] of [['','Раскрытие'],...Object.entries(poseNames)]){
...
function setFold(f,preservePose=false){
  ...
  $('#foldv').textContent=fold.toFixed(2);
...
// взрыв-схема и режимы разбора
$('#explode').oninput = e => { explodeAmount = e.target.value/1000;
  $('#explodev').textContent = explodeAmount.toFixed(2); applyExplode(); };
...
function viewerFrame(){
  const mobile=innerWidth<720,left=mobile?0:310,top=mobile?12:0;
  return {left,top,width:Math.max(48,innerWidth-left),height:mobile?Math.max(120,innerHeight*.54):innerHeight};
}
...
function resetAll(){
  ...
  explodeAmount = 0;
  $('#explode').value = 0; $('#explodev').textContent = '0.00';
  applyExplode();
